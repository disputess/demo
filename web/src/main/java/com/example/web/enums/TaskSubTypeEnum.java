...
public enum TaskSubTypeEnum {
    ...
    //该任务子类型已经去掉
    //该任务子类型已经去掉
    HAVE_ORDER_NO_METER(30, "有单无程", "正常出租且连续3个订单里程低于200米或使用时间低于3分钟的车辆（正常完成一次任务后，清除“有单无程”的属性）"),
    ...
    NO_ORDER_IN_5DAYS(42, "5日以上无单", "最新上架时间距离上次订单已经超过120小时没有订单的车辆。（预约车辆不不算做订单）"),
    ...
    NEW_BIKE_ADDRESS(82, "新车上门", "购买新车然后将新车送到指定地址"),
    ...
    VIP_BIKE_ACCESSORY_DOOR(89, "配件上门", "专享用户发起配件上门"),
    VIP_BIKE_COMPANY_ORDER(821, "企业订购", "企业订购一批专享车,运维将新车送到指定地址"),
    REPAIR(90,"维修",""),
    ...
    public static TaskTypeEnum getTaskType(TaskSubTypeEnum subTypeEnum) {
        int taskType;
        int taskSubType = subTypeEnum.getCode().intValue();
        if (taskSubType < 1000) {
            taskType = getHighestNum(taskSubType);
        } else {
            taskType = taskSubType / 100;
        }
        return TaskTypeEnum.getEnum(taskType);
    }
    ...
    public static Map<Integer,Object> getListTaskSubTypeEnum(){
        Map<Integer,Object> map = new HashMap<>();
        for (TaskSubTypeEnum taskSubTypeEnum : TaskSubTypeEnum.values()) {
            map.put(taskSubTypeEnum.code,taskSubTypeEnum.desc);
        }
        return map;
    }
}
